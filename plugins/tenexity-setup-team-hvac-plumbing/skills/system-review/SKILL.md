...
Read every file in `ABOUT ME/COMPANY/` and `ABOUT ME/USER/`. Score each using `references/scoring-guide.md`. Both layers are in scope for the review — the COMPANY layer (admin-managed) and the USER layer (this user's individual files).

For COMPANY/ files specifically, check:
- `company-profile.md` — company name, ERP named (not left as "Eclipse / P21 / SX.e / other"), branches listed, top vendors mentioned, customer mix described, no default-archetype bracketed prompts remaining
- `voice-and-writing-rules.md` — at least one company-level belief filled in (not the generic default), audience-specific communication patterns described, banned-words and structural-pattern sections intact
- `tools-and-context.md` — ERP specifically named, top vendors with rebate programs listed, sanctioned local-folder convention if applicable
- `specialist-routing.md` — pain-point priorities reordered based on this business

For USER/ files specifically, check:
- `me.md` — name, role, branch, tenure all filled in; "what you write most" section reflects this user's real work
- `my-voice.md` — either explicitly notes "using company default" OR has real overrides; not just leftover template
- `my-priorities.md` — top 3 pain points populated with specific (not vague) detail; 90-day outcomes named
- `my-memory.md` — active entries beyond the install entry

Flag any file that scores Empty, Template, or Shallow as a priority fix. **Distinguish in the report whether the gap is COMPANY-level (needs admin attention) or USER-level (this user can fix themselves).** Recommendations for COMPANY/ go to the admin via the system review report, not to individual users.
...
Read `ABOUT ME/COMPANY/specialist-routing.md` and check `RESOURCES/PLUGINS/`. Flag mismatches:
...
Read `ABOUT ME/USER/my-memory.md` and every `WORK AREAS/*/[*-project]/memory.md`. Look for:

**Recurring corrections** — the user corrected Claude's output multiple times on the same issue. If a correction appears 2+ times and isn't already captured in `COMPANY/voice-and-writing-rules.md` or `USER/my-voice.md`, propose adding it. **Decide which layer**: a correction that should apply to everyone at the company (e.g., "we never use 'one-stop shop'") goes to COMPANY (admin approval). A correction specific to this user's style goes to USER/my-voice.md.

**Common task types** — what this user actually does. Top 3-5 recurring work types (drafting RFQ responses, vendor price-increase emails, AR triage, dead-stock reviews, QBRs). For each, check whether a template exists in `RESOURCES/TEMPLATES/` and whether the relevant work area is being used.

**Missing context** — memory entries revealing gaps ("had to explain our branch codes again," "Claude didn't know we run SX.e," "had to remind Claude that we're in AD buying group"). Identify which file should hold the missing info — usually a COMPANY/ file (admin approves) for company-wide gaps, or a USER/ file for user-specific gaps. Draft the specific line to add.
...
Be direct. Use the writing rules from the user's own `ABOUT ME/COMPANY/voice-and-writing-rules.md` (with their `USER/my-voice.md` overrides applied) — if they've banned SaaS jargon, so should you.

Never pad with observations that don't matter. Minor observations dilute critical ones.

When you flag something critical, be specific about consequences: "Your COMPANY/voice-and-writing-rules.md is still the default template — every email Claude drafts for anyone at this company will sound like a generic AI, not like the business" beats "writing-rules needs attention."
